...
static void Text_Draw( menutext_s *t )
{
	...
	touch.callback = t->generic.id;
	touch.menu = t->generic.parent->id;
	touch.draw = t->generic.flags & QMF_TOUCH;
	
	UI_DrawStringWithTouch( x, y, buff, t->style, color, &touch );
}
...
static void PText_Draw( menutext_s *t )
{
	...
	UI_DrawProportionalString( x, y, t->string, style, color);//, &touch );
}
...
//	trap_DrawTouchArea(320.0f-(((160.0f-width)*uis.scale)), 150.0f*uis.scale,
	trap_DrawTouchArea(320.0f-((menu->generic.x-width)*uis.scale), menu->generic.y*uis.scale,
					   (float)width, (float)PROP_HEIGHT, menu->generic.parent->id, menu->generic.id);
}

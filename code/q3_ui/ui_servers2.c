...
static void ArenaServers_Event( void* ptr, int event ) {
	...
	case ID_PUNKBUSTER:
		...
		{
			UI_ConfirmMenu_Style( "Enable Punkbuster?",  UI_CENTER|UI_INVERSE|UI_SMALLFONT, UI_voidfunc, Punkbuster_ConfirmEnable );
		}
		else
		{
			UI_ConfirmMenu_Style( "Disable Punkbuster?", UI_CENTER|UI_INVERSE|UI_SMALLFONT, UI_voidfunc, Punkbuster_ConfirmDisable );
		}
		break;
	}
}

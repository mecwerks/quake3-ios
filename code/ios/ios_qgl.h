...
//#ifndef VM_COMPATABILITY
	if (device_height > device_width)
		glViewport(x, y, width / (device_scale*(device_height/device_width)), height / (device_scale*(device_height/device_width)));
	else
		glViewport(x, y, width / (device_scale*(device_width/device_height)), height / (device_scale*(device_width/device_height)));
#if !defined(NDEBUG) && defined(QGL_CHECK_GL_ERRORS)
    if (!QGLBeginStarted)
